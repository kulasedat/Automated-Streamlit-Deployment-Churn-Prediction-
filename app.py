...
st.table(pd.DataFrame(df.dtypes))
...
st.cache()
columns_select = st.selectbox("Select All Columns or Select Columns You Want to Drop",("All Columns","Select Columns to Drop"))
...
missing_values = st.selectbox("Select one method for missing value imputation ", ("Drop All Missing Values", "Filling with Either Median or Mode"))
def impute_with_median(df,variable):
    if df[variable].isnull().sum()>0:
        st.write(f"{variable} column has {df[variable].isnull().sum()} missing value and replaced with median:{df[variable].median()}")
...
def impute_with_mode(df,variable):
    if df[variable].isnull().sum() > 0:
        st.write(f"{variable} column has {df[variable].isnull().sum()} missing value and replaced {df[variable].mode()[0]}")
        df[variable+"NAN"] = np.where(df[variable].isnull(), 1, 0)
...
def feature_importance(endogenous, exogenous, n):
    ...
    feature_score = pd.DataFrame(selected.scores_, index=endogenous.columns, columns=["Score"])["Score"].sort_values(ascending=False).reset_index()
...
if standardization == 'Apply Standardization':
    methods = st.selectbox("Select one of the Standardization Methods: ",("StandardScaler", "MinMaxScaler", "RobustScaler"))
...
models_name = st.sidebar.selectbox("Select Model for ML", ("KNN", "XGBoost", "Random Forest"))
params = dict()
def parameter_selection(clf_name):
    ...
    if clf_name == "KNN":
        ...
        st.sidebar.write("Select Parameters")
        n_estimators = st.sidebar.slider("n_estimators", 50, 1000)
        sub_sample = st.sidebar.slider('sub_sample', 0.0, 2.0, 0.1)
        max_depth = st.sidebar.slider("max_depth", 3, 20)
        learning_rate = st.sidebar.slider('learning_rate', 0.01, 1.0, 0.01)
...
def get_classifier(classifier_name,params):
    if classifier_name == "KNN":
        clf_model = KNeighborsClassifier(n_neighbors=params["K"],leaf_size=params["leaf_size"],p=params["p"])
    elif classifier_name == "XGBoost":
        clf_model = XGBClassifier(n_estimators=params["n_estimators"], sub_sample=params["sub_sample"], max_depth = params["max_depth"], learning_rate=params["learning_rate"])
    else:
        clf_model = RandomForestClassifier(n_estimators=params["n_estimators"], max_depth=params["max_depth"],criterion=params["criterion"],max_features=params["max_features"],min_samples_leaf=params["min_samples_leaf"],min_samples_split=params["min_samples_split"] ,random_state=42)
...
if st.button("Prediction of Churn Probality of Top 10 Customers"):
    col_val = ['0', '1']
...
if st.button("Prediction of Churn Probality of Top 20 Loyal Customers"):
    col_val = ['0', '1']
    a = clf_model.predict_proba(X_test)
    b = pd.DataFrame(data=a, columns=col_val)
